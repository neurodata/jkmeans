...
arcM<- rDARC(X,d = 9,k = K,meansIni = matrix(1,2),steps = 5E3,sigma2_ini = 0.1,randomStart = F,ver = 1,useEstep = T)



plotFace(arcM$mu[1,]%*%arcM$EV)
plotFace(arcM$mu[2,]%*%arcM$EV)
plotFace(arcM$mu[3,]%*%arcM$EV)
...
miscError<- function(x){
  rate=sum(x != as.numeric(as.factor(M)))/length(x)
  min(rate,1-rate)
}
