...
\State Compute $ \mathbb{E}(z_{i,k})= \frac{w_{k,i} \delta_{\theta_{k}}(y_i)}{\sum_k w_{k,i} \delta_{\theta_{k}}(y_i)}$ with $w_{k,i} = u_{i,k} v_{k}$; \Comment{E step  based on (\ref{conditional_lik})}
